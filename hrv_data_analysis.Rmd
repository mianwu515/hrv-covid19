...
table(combined_hrv_data$diagnosis)
...
colSums(is.na(combined_hrv_data))
```
```{r}
...
```{r}
## Normalize the data
# combined_hrv_data_norm <- combined_hrv_data %>% mutate(across(c("rmssd", "sdnn", "pnn50", "bpm", "meanrr", "mxdmn",  "mode", "amo", "lf", "hf", "vlf", "lfhf", "total_power", "how_feel", "how_mood", "bmi"), ~(. - mean(.)) / sd(.)))
...
```{r}
## Handle outliers
mean_chol <- mean(Cholesterol_Cirrhosis_without_NA$Cholesterol)
sd_chol <- sd(Cholesterol_Cirrhosis_without_NA$Cholesterol)
mean_pt <- mean(Prothrombin_Cirrhosis_without_NA$Prothrombin)
sd_pt <- sd(Prothrombin_Cirrhosis_without_NA$Prothrombin)

# Calculate the upper and lower bounds for cholesterol and prothrombin
upper_chol <- mean_chol + 3*sd_chol
lower_chol <- mean_chol - 3*sd_chol
upper_pt <- mean_pt + 3*sd_pt
lower_pt <- mean_pt - 3*sd_pt

# Count the number of values that lie outside the bounds
num_outliers_chol <- Cholesterol_Cirrhosis_without_NA %>% filter(Cholesterol > upper_chol | Cholesterol < lower_chol) %>% nrow()
num_outliers_pt <- Prothrombin_Cirrhosis_without_NA %>% filter(Prothrombin > upper_pt | Prothrombin < lower_pt) %>% nrow()
```
...
```{r}
x_scale = combined_hrv_data %>% mutate_if(is.numeric, scale)
head(x_scale, 5)

y_data = as.factor(combined_hrv_data$symptoms_onset)
x_scale
y_data
```
