...
combined_hrv_data <- combined_hrv_data %>%
  select(-c("rr_code", "how_sleep", "city", "country", "rr_data"))
```
...
```{r}
## Normalize the data
hrv_data_norm <- hrv_data %>%
  mutate(across(c("rmssd", "sdnn", "pnn50"), ~(. - mean(.)) / sd(.)))

head(hrv_data_norm, 20)
```

## Data exploration and visualization
```{r}
## Descriptive statistics
summary(hrv_data_norm)
```
```{r}
## Correlation matrix
cor(hrv_data_norm[,c("rmssd", "sdnn", "pnn50", "Age", "BMI")])
```
